...
namespace RescoCLI.Tasks
{
	...
	public class CSharpGeneratorUtilCmd : HARTBBase
    {
		...
		protected override async Task<int> OnExecute(CommandLineApplication app)
		{
			...
			Namespace = string.IsNullOrEmpty(Namespace) ? "HA.Resco" : "";
            if (string.IsNullOrEmpty(FolderPath))
            {
				FolderPath = configuration.CSharpEntitiesFolderPath;
            }
		...
		public void Generat(string folderName, string url, NetworkCredential credentials, string _namespace = "HA.Resco")
		{
			...
			_console.WriteLine($"Total Entities: {entities.Count}");
				...
				foreach (var attribute in entity.Attributes)
				{
					...
					switch (attribute.Type)
					{
						...
						case XrmType.PartyList:
							codeMemberProperty.Type = new CodeTypeReference("EntityReference");
							codeMemberProperty.GetStatements.Add(new CodeMethodReturnStatement(new CodeFieldReferenceExpression(new CodeThisReferenceExpression(), $"GetPropertyValue<EntityReference>(\"{attribute.Name }\")")));
			...
			CodeNamespace optionSetCodeNamespace = new CodeNamespace("HA.Resco");
